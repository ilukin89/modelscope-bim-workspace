...
interface TopBarProps {
  ...
  darkMode: boolean
  demoUserDisplayName: string | null
...
export function TopBar({
  ...
  const accountDisplayName =
    demoUserDisplayName?.replace(/^ModelScope\s+/, "") ?? null
    ...
    <>
      <header className="z-30 grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center border-b border-border bg-panel px-2.5 max-[680px]:grid-cols-[auto_minmax(0,1fr)_auto] max-[380px]:px-1.5">
        ...
        <div className="flex min-w-0 shrink-0 items-center justify-end gap-1.5 max-[380px]:gap-0.5">
          ...
          {accountDisplayName && (
            <div
              className="flex max-w-[170px] items-center gap-1.5 truncate text-[11px] text-foreground/80 max-[900px]:hidden"
              aria-label={`Signed in as ${accountDisplayName}`}
            >
              <UserRound
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-primary hover:bg-primary/8 hover:text-primary max-[420px]:px-1.5 max-[420px]:text-[10px]"
      ...
      <AlertDialog open={signOutDialogOpen} onOpenChange={setSignOutDialogOpen}>
        <AlertDialogContent className="max-w-sm">
          <AlertDialogHeader>
            <AlertDialogTitle>Sign out of demo session?</AlertDialogTitle>
            <AlertDialogDescription>
              This will end the current demo session and return you to the sign
              in screen. Any unsaved local view state may reset.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Stay in workspace</AlertDialogCancel>
            <AlertDialogAction onClick={() => void supabase.auth.signOut()}>
              Sign out
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
